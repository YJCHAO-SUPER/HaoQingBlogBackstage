...
<div id="app">
    ...
    <div class="container clearfix">
        ...
        <h1 style="color: darkblue; font-size: 20px">更新管理员管理</h1><br>
        <form action="/updateManage" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="{{$data->id}}">
            @csrf
            添加管理员：<br>
                        用户名：<input type="text" name="name" value="{{ $data->name }}"><br>
                        密码：<input type="password" name="password" value="{{ $data->password }}"><br>
                        邮箱：<input type="text" name="email" value="{{ $data->email }}"><br>
                        电话号码：<input type="text" name="phone" value="{{ $data->phone }}"><br>

            选择角色：<ul>
                @foreach($all as $v)
                    @if(in_array($v->id,$checked))
                        <li><input type="checkbox" name="role_id[]" value="{{ $v->id }}" checked>{{ $v->display_name }}</li>
                    @else
