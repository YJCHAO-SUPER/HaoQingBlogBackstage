...
<div id="app">
    ...
    <div class="container clearfix">
        ...
        <h1 style="color: darkblue; font-size: 20px">更新角色管理</h1><br>
        <form action="/updateRole" method="post" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{$data->id}}">
            角色英文名称：<input type="text" name="name" value="{{ $data->name }}">
            角色名称：<input type="text" name="roleName" value="{{ $data->display_name }}">
            角色描述：<input type="text" name="description" value="{{ $data->description }}">
            <br>
            权限列表：<ul>
                @foreach($all as $v)
                    @if(in_array($v->id,$checked))
                        <li><input type="checkbox" name="per_id[]" value="{{$v->id}}" checked>{{ $v->display_name }}</li>
                    @else
                        <li><input type="checkbox" name="per_id[]" value="{{$v->id}}">{{ $v->display_name }}</li>
                    @endif
                @endforeach
                     </ul>
            <br>
